...
import {
    ...
    ToastAndroid, Text, Button, ActivityIndicator, StyleSheet, TouchableOpacity, Dimensions, Image, FlatList, ScrollView, TextInput,
    Platform,
    Modal
} from 'react-native';
...
const HomeScreen = ({ navigation }) => {
            ...
            headerRight: () => (
                <View style={{ flexDirection: 'row' }}>
                    <FontAwesome style={{ padding: 10 }} onPress={() => {
                        refresh();
                    }} name="refresh" size={20} backgroundColor="#ffffff"
                        color="#85388d" />
                </View>
            ),
        });
            ...
            AsyncStorage.getItem('userToken').then(value => {
                ...
                messaging()
                    ...
                    .then(token => {
                        ...
                        console.log('sent')
                        fetch("https://trucktaxi.co.in/api/customer/updateToken", requestOptions)
    ...
    const reload = () => {
        ...
        AsyncStorage.getItem('userdata').then(userdata => {
            AsyncStorage.getItem('userToken').then(value => {
                ...
                fetch("https://trucktaxi.co.in/api/customer/getvehicletypes?cityid=" + parseddata.cityid, requestOptions)
                    .then(response => response.json())
                    .then(result => {
                ...
                fetch("https://trucktaxi.co.in/api/customer/getgoodstypes", requestOptions)
                    .then(response => response.json())
                    .then(result => {
                ...
                fetch("https://trucktaxi.co.in/api/customer/gettriptypes", requestOptions)
                    .then(response => response.json())
                    .then(result => {
                        // console.log(result)
                        setshowLoading(false)
    ...
    const loadgoods = () => {
        return goodType.map(good => (
            <SelectItem title={good.goodsname} />
        ))
    }

    const loadpackage = () => {
        return packageType.map(packag => (
            <SelectItem title={<Text>{packag.package} @ Rs.{packag.basefare} </Text>} />
        ))
    }
    const loadintercity = () => {
        return intercityType.map(packag => (
            <SelectItem title={<Text>{packag.basekm} @ Rs.{packag.basefare} </Text>} />
        ))
    }
    const loadnight = () => {
    ...
    const BookingPreview = () => {
        ...
        if (selectedvehcilelist == undefined) {
            ...
            var datetosend = moment(dateobj, "DD/MM/YYYY hh:mm A").format("DD/MM/YYYY")
    ...
    const loadtrips = () => {
        return tripType.map(trips => (
            <SelectItem title={trips.name} />
        ))
    }
    ...
    const onSelectVehicle = (item) => {
        ...
        AsyncStorage.getItem('userdata').then(userdata => {
            AsyncStorage.getItem('userToken').then(value => {
                ...
                fetch("https://trucktaxi.co.in/api/customer/getpackages?vehicleid=" + item.id + "&cityid=" + parseddata.cityid, requestOptions)
                    .then(response => response.json())
                    .then(result => {
                        setshowLoading(false)
                        // console.log(result)
                ...
                fetch("https://trucktaxi.co.in/api/customer/getintercitylist?vehicleid=" + item.id + "&cityid=" + parseddata.cityid, requestOptions)
                    .then(response => response.json())
                    .then(result => {
                        setshowLoading(false)
                        console.log(result)
    ...
    return (
        <ScrollView>
            <View style={{ width: '100%', marginTop: 20 }}>
                <Text style={{ fontSize: 16, color: Color.black, fontFamily: Manrope.SemiBold, paddingHorizontal: 20 }}>
                    Choose Your Vehicle
                </Text>
                <FlatList
                        ...
                        return (
                            <View>
                                <View style={{
                                    ...
                                    <TouchableOpacity key={index} onPress={() => onSelectVehicle(item)}>
                                        <Image
                                            style={styles.vehicleImg}
                                            source={{ uri: item.url }}
                                        />
                                        <Text style={styles.vehicleName}>{item.vehicletype}</Text>
                                        <Text style={styles.vehicleName}> {item.loadcapacity} </Text>
                ...
                {selectedvehcilelist != null && (
                    <View style={styles.typeView}>
                        <Text style={{ paddingStart: 20, paddingVertical: 5, fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.Bold }}>
                            Vehicle Type:{' '}
                        </Text>
                        <TouchableOpacity
                            onPress={() => {
            ...
            <View style={styles.containernormal}>
                ...
                <View style={{ width: '95%', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                    <View style={{ flex: 1, justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                        <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold, paddingVertical: 10 }}>Pickup Location :</Text>
                        <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold, paddingVertical: 10 }}>Drop Location :</Text>
                    </View>
                    <View style={{ flex: 1.5, justifyContent: 'flex-start', alignItems: 'flex-start' }}>
                        {origin != null && origin != undefined && origin != " " ?
                            <View>
                                <View style={{ paddingVertical: 5 }}>
                                    <Text style={{ fontSize: 16, textAlign: 'justify', color: Color.black, fontFamily: Manrope.Bold }}>{initorigin}</Text>
                                    <TouchableOpacity
                                ...
                                <View style={{ paddingVertical: 5 }}>
                                    <Text style={{ fontSize: 16, textAlign: 'justify', color: Color.black, fontFamily: Manrope.Bold }}>{initdrop}</Text>
                                    <TouchableOpacity
                                        onPress={() => navigation.navigate('pickupoint')}>
                                        <Text
                                            style={{
                ...
                <View style={{ marginVertical: 10 }}>
                    <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold, paddingVertical: 5 }}>Date & Time</Text>
                    <TouchableOpacity onPress={() => {
                        setShow(true)
                    }}>
                        <View style={styles.action}>
                            <FontAwesome style={styles.icon} name="calendar-plus-o" color={'#828282'} size={20} />
                            ...
                            <View style={{ backgroundColor: '#fff', borderRadius: 20, paddingTop: 6 }}>
                                <TextInput
                ...
                <View style={{ marginVertical: 0 }}>
                    <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold }}>Goods Type</Text>
                    <View style={styles.action}>
                        <FontAwesome style={styles.icon} name="dropbox" color={'#828282'} size={20} />
                        <Select
                ...
                <View style={{ marginVertical: 10 }}>
                    <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold }}>Fare Type</Text>
                    <View style={styles.action}>
                        <FontAwesome style={styles.icon} name="object-group" color={'#828282'} size={20} />
                        <Select
                ...
                {tripValue == 'Package Fare' ? (
                    <View>
                        <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold }}>Package Fare Type</Text>
                        <View style={styles.action}>
                            <FontAwesome style={styles.icon} name="clock-o" color={'#828282'} size={20} />
                            <Select
                                selectedIndex={packageIndex}
                                value={Packagevalue}
                                style={{ backgroundColor: '#fff', borderColor: '#fff', paddingLeft: 20, width: "80%" }}
                                onSelect={index => {
                ...
                {tripValue == 'Intercity Fare' ? (
                    <View>
                        <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold }}>Intercity Fare Type</Text>
                        <View style={styles.action}>
                            <FontAwesome style={styles.icon} name="clock-o" color={'#828282'} size={20} />
                            <Select
                ...
                {tripValue == 'Night Fare' ? (
                    <View>
                        <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold }}>Night Fare Type</Text>
                        <View style={styles.action}>
                            <FontAwesome style={styles.icon} name="clock-o" color={'#828282'} size={20} />
                            <Select
                ...
                <View style={{ marginVertical: 5 }}>
                    <Text style={{ fontSize: 16, color: Color.lightBlack, fontFamily: Manrope.SemiBold }}>No. Of Trips</Text>
                    <View style={styles.action}>
                        <FontAwesome style={styles.icon} name="dropbox" color={'#828282'} size={20} />
                        <Select
